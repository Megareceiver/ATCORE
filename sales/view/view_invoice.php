...
if (db_num_rows($result) > 0){
	...
	while ($myrow2 = db_fetch($result)){
	     ...
	     //$tax = tax_calculator($myrow2['tax_type_id'],$myrow2['unit_price']*$myrow2['quantity']*(1-$myrow2['discount_percent']),$myrow['tax_included']);
	    $tax = tax_calculator($myrow2['tax_type_id'],$myrow2['unit_price']*$myrow2['quantity']*(1-$myrow2['discount_percent']),$myrow['tax_included']);

	     if( !array_key_exists($tax->id, $item_taxes) ){
	         $item_taxes[$tax->id] = array('name'=>$tax->name,'rate'=>$tax->rate,'value'=>0,'no'=>$tax->code);
	     }
...
label_row(_("Discount Given"), number_format2($myrow["ov_discount"],user_amount_dec()), "colspan=6 align=right","nowrap align=right");
...
foreach ($item_taxes as $tax) {
    ...
    if ($myrow['tax_included']){
        label_row(_("Included")." ".$tax['name']." ".$tax['no']. "(" . $tax['rate'] . "%) ". ": $value ", '', "colspan=6 align=right", "align=right");
    } else {
        label_row($tax['name'] . " (" . $tax['rate'] . "%) ".$tax['no'], $value, "colspan=6 align=right", "align=right");
    }
}
// bug($myrow);
// bug($myrow);die;
$display_total = number_format2($myrow["ov_freight"]+$myrow["ov_gst"]+$myrow["ov_amount"]+$myrow["ov_freight_tax"], user_amount_dec());
