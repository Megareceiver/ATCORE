...
if (isset($_GET['AddedID'])) {
	...
	hyperlink_params(site_url()."/sales/inquiry/sales_deliveries_view.php", _("Select Another &Delivery For Invoicing"), "OutstandingOnly=1");
...
function update_items(){
    ...
    $line_no = 0;
//     bug($_SESSION['Items']->line_items);
//     bug($_POST['line']);die;
    foreach ($_POST['line'] AS $ite){
...
//         bug($ite);
        ...
        $line_no++;
//         foreach ($_SESSION['Items']->line_items AS $car_index=>$cart_item){
// //             if( $ite['stock_id']== $cart_item->stock_id ){
//                 foreach ($update_fields AS $field){
//                     if( $field=='discount_percent' ){
//                         $ite[$field] = $ite[$field]/100;
//                     }
//                     if( isset($ite[$field]) ){
//                         $_SESSION['Items']->line_items[$car_index]->$field = $ite[$field];
//                     }

//                 }
// //                 bug($_SESSION['Items']->line_items[$car_index]); die;
//                 unset($_POST['line'][$index]);
// //             }
//         }
    }
//     bug($_SESSION['Items']->line_items); die;


//     if( count($_POST['line']) > 0 ){
//         foreach ($_POST['line'] AS $index=>$ite){
//             $_SESSION['Items']->add_to_cart_new(0, count($_SESSION['Items']->line_items), $ite['stock_id'], $ite['quantity'], $ite['price'], $disc=null, $ite['qty_done'], 0, $ite['item_description']);
...
//     foreach ($_SESSION['Items']->line_items AS $line_no=>$cart_item){
//         if( !in_array($cart_item->stock_id,$item_use) ){
//             $_SESSION['Items']->remove_from_cart($line_no);
//         }
//     }

//     bug( count($_POST['line']) );
//     bug( count($_SESSION['Items']->line_items) );
//     die;
}
...
end_table(1);
*/
$ci->controller_load('sale');
echo $ci->sale->customer_invoice();
div_end();
...
submit_center_last('process_invoice', _("Process Invoice"),_('Check entered data and save document'), false);
