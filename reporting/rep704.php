...
//                     $ref = $ci->db->where(array('trans_no'=>$myrow["type_no"],'type'=>$myrow["type"]))->get('bank_trans')->row();
				    $ref = $ci->db->select('ref')->where(array('trans_no'=>$myrow["type_no"],'type'=>$myrow["type"]))->get('bank_trans')->row();

				    $reference = (is_object($ref) && isset($ref->ref)) ? $ref->ref : NULL;
...
// 			        $reference = $invoice->reference;
				...
				$rep->TextCol(1, 2, $reference,$corr=-3, $r=0, $border=0);
				$rep->TextCol(2, 3,	$myrow['type_no'], 0);
