...
if( $supp_trans->document_upload > 0 ){
    $doc_upload = get_instance()->db->where('id',$supp_trans->document_upload)->get('documents_bookkeepers')->row();
    $data_uploaded = unserialize($doc_upload->data);
    if( isset($data_uploaded['file']) && strlen($file=$data_uploaded['file']) > 0 ){
        label_cells(_("Uploaded Document"), anchor(config_item('assets_domain').$data_uploaded['file'],'View '.$supp_trans->document_upload,'target="_blank"'), "class='tableheader2'");
    }
...
display_supp_trans_tax_details2($supp_trans, 1);
