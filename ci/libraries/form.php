...
class form extends ci {
	...
	static function formInputGroup($template=null, $params=null){
		...
		$only_input = false;
		if( isset($colunm_per[1]) && intval($colunm_per[1])==12 ){
		    $only_input = true;
		}
        ...
        $html = NULL;
		if( $only_input ){
            $html = '<div class="form-group" '.($id ? ' id="'.$id.'" ' : NULL).' >'.self::formInput($template,$params)."</div>";
	...
	static function formInput($template=null, $params=null){
		...
		if ( isset($input_field['onchange_ajax']) ){
		    $onchange_ajax = $input_field['onchange_ajax'];
		}
...
// 			    return self::input_text($name,$value,$attributes,$disabled);
			    break;
			case 'number':
				$attributes = array('class'=>'number form-control');
				$value = price_format($value);
		    ...
		    case 'BANK_ACCOUNTS':
		        ...
		        $attributes = '';
		        if( $onchange_ajax ){
		            $attributes = array('class'=>'autosubmit');
		        }
		        return self::input_selectbox($name,$value,array('options'=>$banks),$attributes);
// 		        return self::input_bank_account($name,$value); break;
		    ...
		    case 'options':
		        ...
		        return $ci->finput->options($name,$options,$value,NULL,'combo2 form-control',null,$submit_on_change);
		        break;
		        break;
// 		    case 'stock_search':
// 		        return $ci->finput->product_items(null,$name, $value); break;
	...
	static function input_hidden($template){
	    $name = ( isset($template['name']) )?$template['name']:null;
	    $value = ( isset($template['value']) )?$template['value']:null;
	    return self::hidden($name,$value);
	}
