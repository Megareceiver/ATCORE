...
class reporting {
	...
	function __construct($front_report=false){
		global $ci;
		$this->ci = $ci;
        if ( !$front_report && in_array(input_val('REP_ID'), array(801,802,803,102,107,109,110,112,113,202,209,210,211,601)) ){
...
//                 $this->ci->credit_note->cn_sale_print($this);
                break;
            case 201:
...
//                 $this->file->content_height = $this->file->height-20;
                break;
            case 210:
...
// 		    $footer['height'] = $this->tcpdf->getPageHeight() - $this->tcpdf->GetY();
	...
	private function footer_html($height_full=false){
	    ...
	    foreach ($this->items AS $ite){
	        ...
	        $tax_amount = 0;
	        if( $ite->tax_type_id ){
	            $tax_detail = $this->ci->api_membership->get_data('taxdetail/'.$ite->tax_type_id);
