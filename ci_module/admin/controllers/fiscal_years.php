...
class AdminFiscalYears {
    ...
    function __construct() {
        ...
        $this->ci = $ci;
        $this->db = $ci->db;
//         $this->page_security = 'SA_FISCALYEARS';
...
//         $this->report = $this->ci->module_control_load('report',null,true);
    ...
    private function submit(){
        ...
        if ( $_POST['UPDATE_ITEM'] &&  intval($selected_id) > 0){
    ...
    private function delete($id=0){
        ...
        if( !$delete_confim && ! $delete_purgin ) {
            ...
            if( !$this->model->check_is_last($year_select->id) ){
                ...
                $this->ci->temp_view('delete_msg_popup', array('id'=>'delete-fiscalyear','class'=>'errormsg','title'=>'Deleting a Fiscal Year'),'error_popup','admin',true);
                return false;
            }
