...
//         bug($ob_item);die;
    ...
    if ( !$this->db->field_exists('permit', 'supp_trans')){
        ...
        if( !$this->db->table_exists('documents_bookkeepers') ){
            $this->db->query("CREATE TABLE `documents_bookkeepers` (
              `id` int(11) NOT NULL AUTO_INCREMENT,
        ...
        if ( !$this->db->field_exists('document_upload', 'debtor_trans')){
            $this->db->query(" ALTER TABLE `debtor_trans` ADD `document_upload` INT(11) NULL DEFAULT 0;");
        }

        if ( !$this->db->field_exists('document_upload', 'supp_trans')){
            $this->db->query(" ALTER TABLE `supp_trans` ADD `document_upload` INT(11) NULL DEFAULT 0;");
        }
    }
...
//         $fix->backDB = get_instance()->load_db(array('dbname'=>'at_avdiscovery_160801'),false,true);
// //         $fix->customer_supplier_payment(true);

//         $fix->restore_customer_payment(true);
// bug($fix);die;
//         bug($query->num_rows());
//         bug($query->result());
//         bug($trans_lost);
