...
class MaintenanceOpening {
    ...
    function bank($pay_type = 'bank'){
        ...
        $exist_date = $this->ci->db->select('tran_date')
                        ->where(array('amount !='=>0,'type'=>'bank'))
                        ->get('opening_gl',1)->row();

        if( $exist_date && isset($exist_date->trans_date) AND !empty($exist_date->trans_date) AND $exist_date->trans_date != "0000-00-00" ){
            $tran_date = date('d-m-Y',strtotime($exist_date->trans_date) );
        } else {
        ...
        echo '<tr><td colspan=2 width=90%></td><td class="textright" >Amount</td><td></td></tr>';
//         bug($bank_account_posting);die;
        foreach ($bank_account_posting AS $group=>$bank){
            ...
            echo '<td>'.$bank->bank_curr_code.'</td>';
            echo '</tr>';
        }
...
//         echo '</tr>';
    ...
    function edit_button($item=NULL){
        $ci = get_instance();

        $html = '<a class="ajaxsubmit" href="'.site_url($ci->uri->uri_string()).'/view?edit='.$item->id.'" >'.((isset($item->allocation) && abs($item->allocation) > 0) ? 'View' : 'Edit' ).'</a>';


        if( !isset($item->allocation) || $item->allocation==null || abs($item->allocation)==0 ){
//             $html .= '  <button class="remove ajaxsubmit" title="remove item" value="'.$item->id.'" type="button" name="_remove" ><span>Remove</span></button>';
            $html .= ' <a class="ajaxsubmit" href="'.site_url($ci->uri->uri_string()).'?remove='.$item->id.'" >Remove</a>';
        }
    ...
    var $table_view = array(
        ...
        'items_action'=>array(NULL,'center','gl_ob_edit','1')
    );
...
// bug($data);die;
        ...
        echo '<div class="center"><a title="Add new item" class="ajaxsubmit" href="'.site_url($ci->uri->uri_string()).'/add"><img height="12" src="'.site_url('themes/template/images/ok.gif').'"> Add </a></div>';
