...
if (isset($_GET['AddedID'])) {
    ...
    // display_note(print_document_link($trans_no, _("&Print This Voucher 2"), true, ST_GLPAYMENT, false, 'prtopt'));
    submenu_print(_("&Print This Voucher"), ST_GLPAYMENT, $trans_no, 'prtopt');
...
if (isset($_GET['AddedDep'])) {
    ...
    br();
    // display_note(print_document_link($trans_no, _("&Print This Voucher"), true, ST_GLDEPOSIT, false, 'download'));
    submenu_print(_("&Print This Voucher"), ST_GLDEPOSIT, $trans_no, 'prtopt');
...
function check_trans()
{
    ...
    if (isset($_POST['settled_amount']) && in_array(get_post('PayType'), array(
        PT_SUPPLIER,
        PT_CUSTOMER
    )) && (input_num('settled_amount') <= 0)) {
...
if (isset($_POST['Process']) && ! check_trans()) {
    ...
    $trans = write_bank_transaction($trans_type, $trans_no, $_POST['bank_account'], $_SESSION['pay_items'], $_POST['date_'], $_POST['PayType'], $_POST['person_id'], get_post('PersonDetailID'), $_POST['ref'], $_POST['gst'], $_POST['memo_'], true, input_num('settled_amount', null), $tax_include = $_POST['tax_inclusive'], $_POST['cheque']);
...
function check_item_data()
{
    ...
    /*
     * allow enter negative
     */
    $amount = floatval(input_post('amount'));
    $amount_negative = floatval(input_post('amount_negative'));
    if( $amount <= 0 AND $amount_negative > 0){
        $_POST['amount'] = -$amount_negative;
    }
    if (! check_num('amount')) {
...
function handle_new_item()
{
    ...
    $_POST['amount_negative'] = $_POST['amount'] = NULL;
    line_start_focus();
}
...
$voided = false;
// bug('id='.$_SESSION['pay_items']->order_id);
...
//     submit_center_last('Process', $_SESSION['pay_items']->trans_type == ST_BANKPAYMENT ? _("Process Payment") : _("Process Deposit"), '', true);
    submit_center_last('Process', $_SESSION['pay_items']->trans_type == ST_BANKPAYMENT ? _("Process Payment") : _("Process Deposit"), '', false);
    // submit_center_last('Process', $_SESSION['pay_items']->trans_type==ST_BANKPAYMENT ?
    // _("Process Payment"):_("Process Deposit"), '',false);
}
