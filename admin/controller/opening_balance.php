...
class openingbalance_controller extends controller {
	...
	function gl_account($account=array()){
		...
		$exist_date = $ci->db->select('tran_date')
		              ->where(array('amount !='=>0,'type'=>1))->get('opening_gl')->row();

		if( $exist_date && isset($exist_date->tran_date) AND !empty($exist_date->tran_date) AND $exist_date->tran_date != "0000-00-00"){
		...
		foreach ($gl_account_posting AS $group=>$items){
                ...
                if( $i <= 1 ){
                    echo '<tr><td style="width:90%;"> </td><td class="textright" >Debit</td><td class="textright" >Credit</td></tr>';
                }
			...
			if( $items ){
				...
				foreach ($items as $code=>$item) {
					...
					echo '<td><input class="textright" type="text\" name="debit['.$code.']" value="'.money_total_format($item['debit']).'" ></td>';
					echo '<td><input class="textright" type="text\" name="credit['.$code.']" value="'.money_total_format($item['credit']).'" ></td>';
					echo '</tr>';
