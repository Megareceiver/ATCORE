...
} elseif ($ci->input->get('reinvoice')) {
// 	$trans_no = $ci->input->get('reinvoice');
// 	$cart = new allocation(ST_CUSTPAYMENT,$trans_no);
// 	$myrow = get_customer_trans($trans_no, ST_CUSTPAYMENT);
// 	$charge = get_cust_bank_charge(ST_CUSTPAYMENT, $trans_no);
// // 	/* do repost */
// bug($myrow);
// die('begin repost cus payment');
// 	$payment_no = write_customer_payment($trans_no, $myrow["debtor_no"], $myrow["branch_code"],
// 		$myrow["bank_act"], $myrow['tran_date'], $myrow["reference"],
// 		$myrow['Total'] - $myrow['ov_discount'], $myrow['ov_discount'], get_comments_string(ST_CUSTPAYMENT,$trans_no), $myrow["rate"], get_cust_bank_charge(ST_CUSTPAYMENT, $trans_no), $myrow['bank_amount']+$charge,$myrow['cheque']);
...
if (get_post('AddPaymentItem') && can_process()) {
	...
	$payment_no = write_customer_payment($_SESSION['alloc']->trans_no, $_POST['customer_id'], $_POST['BranchID'],
		...
		input_num('amount'), input_num('discount'), $_POST['memo_'], 0, input_num('charge'), input_num('bank_amount', input_num('amount')),$_POST['cheque']);

	$_SESSION['alloc']->trans_no = $payment_no;
	$_SESSION['alloc']->write();

	if( isset($_POST['source_ref']) ){
	    update_source_ref($_SESSION['alloc']->type,$payment_no,$_POST['source_ref']);
	}
...
start_form();
	...
	text_cells('Cheque No.','cheque',null);

	if( defined('COUNTRY') && COUNTRY==65 ){
	    $ci->finput->text('Customer Ref','source_ref',null,'row_echo');
	}
	table_section(3);
	...
	if ($new)
		submit_center('AddPaymentItem', _("Add Payment"), true, '', false);
	else
