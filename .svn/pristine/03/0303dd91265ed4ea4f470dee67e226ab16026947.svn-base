...
if (db_num_rows($result) > 0){
	...
	while ($myrow2 = db_fetch($result)){
		...
		if( !array_key_exists($tax->id, $item_taxes) ){
		    $item_taxes[$tax->id] = array('name'=>$tax->name,'rate'=>$tax->rate,'value'=>0);
		}
...
foreach ($item_taxes as $tax) {
    ...
    if ($myrow['tax_included']){
        label_row(_("Included") . " " . $tax['name'] . " (" . $tax['rate'] . "%) ". ": $value ", '', "colspan=6 align=right", "align=right");
    } else {
        label_row($tax['name'] . " (" . $tax['rate'] . "%)", $value, "colspan=6 align=right", "align=right");
    }
}

$display_total = number_format2($myrow["ov_freight"]+$myrow["ov_amount"]+$myrow["ov_freight_tax"]+$myrow["ov_gst"],user_amount_dec() );
