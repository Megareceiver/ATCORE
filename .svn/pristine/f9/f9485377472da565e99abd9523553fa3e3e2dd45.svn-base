...
function handle_submit(&$selected_id){
	...
	if (!can_process())
		return;
	//
	if (isset($_POST['gst'])  && $_POST['gst'] == 0)
		$cus_tax_id = -1;
	else if( isset($_POST['customer_gst_03_type']) )
		$cus_tax_id = $_POST['customer_gst_03_type'];
	else
		$cus_tax_id = 0;
...
function customer_settings($selected_id) {
	...
	global $SysPrefs, $path_to_root, $auto_create_branch, $ci;
...
// 		bug($myrow);die;
	...
	start_outer_table(TABLESTYLE2, null, 2, 0, false,'50%');
	table_section(1);
	...
	///radio///
	if(isset($_POST["customer_tax_id"]) && $_POST["customer_tax_id"] > 0) {
		echo '<tr><td><input type="radio" name="gst" value="0" >&nbsp;&nbsp;GST by Product Setting</td><td>&nbsp;</td></tr>';
		echo '<tr><td><input type="radio" name="gst" value="1" checked>&nbsp;&nbsp;GST by Customer</td>';
		echo customer_tax_list_row(_("customer_tax_id"), 'customer_tax_id', $_POST['customer_tax_id']);
	} else {
		if(isset($_POST['submit']) && isset($_POST['gst']) &&  $_POST['gst'] == 1) {
			echo '<tr><td><input type="radio" name="gst" value="0" >&nbsp;&nbsp;GST by Product Setting</td><td>&nbsp;</td></tr>';
			echo '<tr><td><input type="radio" name="gst" value="1" checked>&nbsp;&nbsp;GST by Customer</td>';
		}else {
			echo '<tr><td><input type="radio" name="gst" value="0" '.(($disable) ? $disable : 'checked').' >&nbsp;&nbsp;GST by Product Setting</td><td>&nbsp;</td></tr>';
			echo '<tr><td><input type="radio" name="gst" value="1" '.$disable.'>&nbsp;&nbsp;GST by Customer</td>';
		}
	...
	}elseif (isset($auto_create_branch) && $auto_create_branch == 1)
	{
		...
		//gst_radio_row( _("Sales Person:"), 'salesman', null);
	...
	if ($selected_id)  {
		...
		echo '<td class="label" >'._('Customer branches').':</td>';
	  	hyperlink_params_td(site_url() . "/sales/manage/customer_branches.php",
