...
// die('iam hee');
		...
		$this->rep = $this->report->front_report(_('Supplier Invoice'),$this->order_report_table);
...
// bug($tran);die;
	    $this->rep->params = array(
	        ...
	        'delivery_info'=>array(
	            'Order To'=>"",
	            'Deliver To'=>$tran->supp_name
	        ),
	        'contact'=>"",
	        'aux_info' => array (
	            "Supplier Reference"          =>  $tran->reference,
	            _ ( "Sales Person" )            =>  NULL,
	    ...
	    $this->rep->params['delivery_info']['Order To'] = $this->rep->company['coy_name'];
	    //         $this->rep->params['delivery_info']['Deliver To'] = $tran->supp_name;
...
// 	    $items = $this->customer_trans_model->trans_detail('*',array('debtor_trans_type'=>ST_SALESINVOICE,'debtor_trans_no'=>$tran->trans_no));
	    if( empty($tran->items) ){
	        ...
	        foreach ($tran->items AS $detail){
	            $detail->quantity_ordered = 1;
	            $line_price = $detail->unit_price * $detail->quantity_ordered;
	            $Net = $line_price;
	            ...
	            $this->rep->TextCol(4, 5,	number_format2($sign*$detail->quantity_ordered,get_qty_dec($detail->stock_id)), -21);
