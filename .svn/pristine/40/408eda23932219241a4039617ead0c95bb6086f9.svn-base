...
$mobile_document = input_val('document');
if( is_numeric($mobile_document) ){
    $mobile_data = $ci->db->where('id',$mobile_document)->get('documents_bookkeepers')->row();
    $document_data = unserialize($mobile_data->data);
}
// bug($document_data);die;
//----------------------------------------------------------------------------------------
...
if (isset($_GET['AddedID'])){
	...
	hyperlink_params( site_url("/purchasing/supplier_payment.php"), _("Entry supplier &payment for this invoice"),
		"PInvoice=".$invoice_no);

	hyperlink_params($_SERVER['PHP_SELF'], _("Enter Another Invoice"), "New=1");

	hyperlink_params(site_url()."/admin/attachments.php", _("Add an Attachment"), "filterType=$trans_type&trans_no=$invoice_no");

	display_footer_exit();

} else if ( $ci->input->get('reinvoice') ) {
    $trans_no = clean_val($ci->input->get('reinvoice'));
...
//     $invoice_no = add_supp_invoice($_SESSION['supp_trans']);
   ...
   meta_forward( site_url()."/gl/view/gl_trans_view.php?type_id=".ST_SUPPINVOICE."&trans_no=".$trans_no);
...
if (isset($_GET['New'])){
	...
	if( is_numeric($mobile_document) ){
	    ...
	    $_SESSION['supp_trans']->supplier_id = $document_data['supplier_id'];
	    $_SESSION['supp_trans']->reference = $document_data['ref'];
	    $_SESSION['supp_trans']->supp_reference = $document_data['supplier_bill_no'];

// 	    $_POST['amount'] = floatval($document_data['supplier_amount']);

	    if( isset($document_data['type']) ){
	        $ci->db->from('sys_expense_type AS exp_type')->where('id',$document_data['type']);
	        $ci->db->join('chart_master AS gl','gl.account_code=exp_type.gl_account');
	        $gl = $ci->db->select('gl.account_name, gl.account_code')->get()->row();

	        if( is_object($gl) ){
// 	            $_POST['gl_code'] = $expense_type->gl_account;
	            $_SESSION['supp_trans']->add_gl_codes_to_trans(0,$gl->account_code, $gl->account_name,0, 0,floatval($document_data['supplier_amount']),'document upload #'.$mobile_document);
	        }
	    }
	}
}
...
if (isset($_POST['AddGLCodeToTrans'])){
	...
	if ($input_error == false){
		$_SESSION['supp_trans']->add_gl_codes_to_trans($ci->input->post('tax_id'),$_POST['gl_code'], $gl_act_name,
...
if ($_POST['supplier_id']=='')
    ...
    if ( $mobile_document < 1 ){
        display_grn_items($_SESSION['supp_trans'], 1);
    }


	display_gl_items($_SESSION['supp_trans'], $mobile_document ? 0 : 1);
...
hidden('document_upload',$mobile_document);

br();
submit_center('PostInvoice', _("Enter Invoice"), true, '', 'default');
// submit_center('PostInvoice', _("Enter Invoice"), true, '', false);
br();
