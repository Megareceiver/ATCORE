...
//             	    ($supplier->purchase_account ? $supplier->purchase_account : $stock_gl_code["cogs_account"]);
            	...
            	if (!is_inventory_item($grn->item_code)) { /* Service item */
                    ...
                    $old_value = get_tax_free_price_for_item($grn->item_code, $grn->this_quantity_inv * $old_price, $cart->supplier_tax_id, $supp_trans->tax_included);
