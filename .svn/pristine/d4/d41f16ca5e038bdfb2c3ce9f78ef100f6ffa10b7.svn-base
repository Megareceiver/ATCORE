...
function credit_link($row)
{
	...
	$link = NULL;
	if ( $row['type'] == ST_SALESINVOICE && $row["Outstanding"] > 0 ){
	//if( $row['type'] == ST_SALESINVOICE && round2($row["TotalAmount"]-$row["Allocated"],2) >= 0.01 ) {

	    $link = pager_link(_("Credit This") , "/sales/customer_credit_invoice.php?InvoiceNumber=". $row['trans_no'], ICON_CREDIT);
	}
	return $link;
}
...
function edit_link($row)
{
    ...
    if( $row['type']==ST_SALESINVOICE && $row['Allocated'] > 0 ){
        return null;
    } else {
...
function prt_link($row)
{
  	...
  	elseif ($row['type'] == ST_SALESINVOICE){
  	    $html = print_document_link($row['trans_no']."-".$row['type'], _("Print"), true, $row['type'], ICON_PRINT);
  	    if( defined('COUNTRY') && COUNTRY==60 ){
  	        $html.= print_document_link($row['trans_no']."-".$row['type'], _("Print"), true, $row['type'], ICON_PRINT,'icons_printtaxinvoice', null, 0, 0,1);
  	    }
...
$cols = array(
	...
	_("#") => array('fun'=>'trans_view_customer', 'ord'=>''),
