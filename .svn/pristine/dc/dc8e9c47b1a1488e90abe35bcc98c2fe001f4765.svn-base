...
//         $this->report = $ci->module_control_load('report',null,true);
    ...
    function delivery_print($pdf){
        ...
        $this->rep->SetHeaderType('TemplateDelivery');
        for ($i = $from; $i <= $to; $i++) {
...
//     );
    var $delivery_report_table = array(
            'space1'=>array(' ',2),
            'stock_id'=>array('Item Code',60),
            'description'=>array('Item Description' ,350),
            'qty'=>array('Quantity' ,420,'center'),
            'units'=>array('Units',460,'center'),
            'discount_percent'=>array('Discount %',515,'right'),
    );
    ...
    private function pdf_print($tran=NULL){
        ...
        if( empty($items) ){
            ...
            foreach ($items AS $detail){
                ...
                $this->rep->TextCol(5, 6,	number_total($detail->discount_percent*100) . "%" , -2);
                $this->rep->NewLine();
            }
            ...
            $this->rep->Font('bold');
            $this->rep->TextCol(3, 5,	_('TOTAL DELIVERY INCL. GST'));
            $this->rep->TextCol(5, 6,	number_total( $tran->tax_included ? $SubTotal :$SubTotal+$taxTotal ));


            $this->rep->NewLine(-1);
            $this->rep->TextCol(3, 5,	_('TOTAL DELIVERY EX GST'));
            $this->rep->TextCol(5, 6,	number_total($tran->tax_included ? $SubTotal-$taxTotal: $SubTotal));
            $this->rep->Font();
            if( abs($discountTotal) != 0 ){
                $this->rep->NewLine(-1);
                $this->rep->TextCol(3, 5,	_('Discount Given'));
                $this->rep->TextCol(5, 6,	number_total($discountTotal));
            }

            if( count($taxes) > 0 ) foreach ($taxes AS $tax){
                $this->rep->NewLine(-1);
                $this->rep->TextCol(3, 5,	$tax['name'].' Amount');
                $this->rep->TextCol(5, 6,	number_total($tax['amount']) );

            }

            if( abs($shippingTotal) != 0 ){
                $this->rep->NewLine(-1);
                $this->rep->TextCol(3, 5,	_('Shipping'));
                $this->rep->TextCol(5, 6,	number_total($shippingTotal));
            }
            $this->rep->NewLine(-1);
            $this->rep->TextCol(3, 5,	_(' Sub-total'));
            $this->rep->TextCol(5, 6,	number_total($SubTotal));
