...
if (isset($_GET['AddedID']))
{
	...
	display_notification_centered(_("The department has been entered."));
...
if (isset($_GET['UpdatedID']))
{
	...
	display_notification_centered(_("The department has been updated."));
	safe_exit();
}
...
if (isset($_GET['ClosedID']))
{
	...
	display_notification_centered(_("The department has been closed. There can be no more changes to it.") . " #$id");
	safe_exit();
}
...
if (isset($_POST['delete']))
{
	...
	{
		display_error(_("This department cannot be deleted because it has already been processed."));
		set_focus('ref');
		$cancel_delete = true;
	}
...
{
	$_POST['dimension_tags'] = array();
	ref_row(_("Class Reference:"), 'ref', '', $Refs->get_next(ST_DIMENSION));
}
...
if (isset($_POST['closed']) && $_POST['closed'] == 1)
	display_note(_("This Department is closed."), 0, 0, "class='currentfg'");

if ($selected_id != -1)
{
	echo "<br>";
	submit_center_first('UPDATE_ITEM', _("Update"), _('Save changes to department'), 'default');
	if ($_POST['closed'] == 1)
		submit('reopen', _("Re-open This Department"), true, _('Mark this department as re-opened'), true);
	else
		submit('close', _("Close This Department"), true, _('Mark this department as closed'), true);
	submit_center_last('delete', _("Delete This Department"), _('Delete unused department'), true);
}
else
{
