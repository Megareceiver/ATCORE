            ...
            if( isset($tax->items) && count($tax->items) >0 ) {
                ...
                foreach ($tax->items AS $ite){
                    ...
                    if( $ite->type=='S' || $ite->type=='DS' || $ite->type=='CCN' || $ite->type=='SBD' ){
                        ...
                        $val_5b += in_array($tax->no, $check_5b) ? $sale_gst : 0;
                        $val_6b += in_array($tax->no, $check_6b) ? $sale_gst : 0;
                    } else if ($ite->type=='P' OR $ite->type=='BP' || $ite->type=='SCN' OR  $ite->type=='PBD' ){
                        ...
                        $val_5b += in_array($tax->no, $check_5b) ? $purchase_gst : 0;
                        $val_6b += in_array($tax->no, $check_6b) ? $purchase_gst : 0;

                    }

//                     if ( ($ite->type=='S' OR $ite->type=='CCN') AND $tax->sales_gl_code != 2150  ){
